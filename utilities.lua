...
function XM:GroupMemberList(groupType)
    local partySize = 5
    local raidGroupCount = 8

    local groupMaxSize = groupType == "raid" and partySize*raidGroupCount or partySize

    local members = {}
    local players = {} --unitid, unitname
    local pets = {} --unitid, unitname, ownerid

    local i,j,k = 1,1,1
    local playerName, petName
    while (i <= groupMaxSize) do
        playerName, _ = UnitName(groupType..i)
        if (playerName) then
            j = #players + 1
            players[j] = {}
            players[j].ID = UnitGUID(groupType..i)
            players[j].NAME = playerName
            petName, _ = UnitName(groupType.."pet"..i)
            if (petName) then
                k = #pets + 1
                pets[k] = {}
                pets[k].ID = UnitGUID(groupType.."pet"..i)
                pets[k].NAME = petName
                pets[k].OWNERID = UnitGUID(groupType..i)
                pets[k].OWNERNAME = UnitName(groupType..i)
            end
        else
            i = groupMaxSize + 1
        end
